...
export class ProductsListComponent {
  ...
  inStock: number = this.products.filter((p)=>p.is_in_inventory).length
  outOfStock: number = this.products.filter((p)=>!p.is_in_inventory).length
  ...
  updateFilter(myEvent: string) {
    this.selected=myEvent
    if (myEvent == 'All')
    {
    ...
    {
      this.currentProducts = this.products.filter((p)=>p.is_in_inventory===Boolean(myEvent))
    }  
    if (this.search != "")
    {
      this.currentProducts = this.currentProducts.filter((p)=>p.name.includes(this.search))
    }
  }
  ...
  updateSearch(data: string) {
    ...
    if (this.search == "")
    {
      ...
      {
        this.currentProducts = this.products.filter((p)=>p.is_in_inventory===Boolean(this.selected))
      }
    }
    else 
    {
